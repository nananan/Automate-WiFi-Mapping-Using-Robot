...
root.mainloop()
	...
	# self.display_ap.grid_propagate(0)
	# scrollbar_ap = MyScrollbar(frame_ap, height=180, command=self.display_ap.yview)
	# scrollbar_ap.pack(side=RIGHT)
	# # scrollbar_ap.config(command=self.display_ap.yview)
	# def y_set(*args):
	# 	scrollbar_ap.set(*args)
	# self.display_ap.configure(yscrollcommand=y_set)
